...
std::vector<uint8_t> Montador::montar(const std::string& caminhoArquivoFonte) {
    ...
    */
    ...
    while (std::getline(arquivoFonte, linha)) {
        ...
        // remover espoaços em brancos no final do operando
        operando.erase(operando.find_last_not_of(" \t") + 1);

        // --- DEBUG: Imprimir o que foi decomposto ---
        std::cout << "Linha: '" << linha << "'\n";
        std::cout << "  > Etiqueta: '" << etiqueta << "'\n";
        std::cout << "  > Instrucao: '" << instrucao << "'\n";
        std::cout << "  > Operando: '" << operando << "'\n";
        std::cout << "---------------------------\n";
        ...
        if (instruçoesFormato1.count(instrucao)) {
            ...
            if (operando[0] == 'C') {
                contadorEndereco += (operando.length() - 3); // Ignora C''
            } else if (operando[0] == 'X') {
                contadorEndereco += (operando.length() - 3) / 2; // Ignora X'' e 2 hex = 1 byte
            }
        }
    }

    // Começar próxima passada
